...
def read_vendors_products(
    ...
    response = schemas.ResponseVendorsProduct(
        **{"count": count, "data": vendors_product}
    )
    return response
...
def create_vendors_product(
    *,
    ...
    vendor = crud.vendors.get_by_user(db=db, id_user=current_user.id)
    if not vendor:
        raise HTTPException(status_code=400, detail="You are not vendor")
...
def read_vendors_product_by_id(
    ...
    if not vendors_product:
        raise HTTPException(status_code=400, detail="VendorsProduct not found")
    return vendors_product
...
def update_vendors_product(
    *,
    ...
    if not vendors_product:
        raise HTTPException(
            status_code=404, detail="vendors_product not found",
        )
    vendors_product = crud.vendors_product.update(
        db, db_obj=vendors_product, obj_in=vendors_product_in
    )
    return vendors_product
