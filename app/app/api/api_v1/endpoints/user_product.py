...
def read_user_products(
    ...
    if crud.user.is_superuser(current_user):
        ...
        count = crud.user_product.get_count(
...
def read_user_product_by_id(
    ...
    if not user_product:
        raise HTTPException(status_code=400, detail="UserProduct not found")
    return user_product
...
def update_user_product(
    *,
    ...
    if not user_product:
        raise HTTPException(
            status_code=404, detail="user_product not found",
        )
    user_product = crud.user_product.update(
        db, db_obj=user_product, obj_in=user_product_in
    )
    return user_product
