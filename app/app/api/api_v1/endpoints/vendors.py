...
def read_vendorss(
    ...
    Retrieve vendors.
    """
    vendors = crud.vendors.get_multi(
...
def create_vendors(
    *, db: Session = Depends(deps.get_db), vendors_in: schemas.VendorsCreate,
) -> Any:
    """
    Create new vendors.
    """
...
def read_vendors_by_id(
    ...
    if not vendors:
        raise HTTPException(status_code=400, detail="Vendors not found")
    return vendors


@router.get("/me", response_model=schemas.Vendors)
def read_vendors_me(
    current_user: models.Vendors = Depends(deps.get_current_active_user),
    db: Session = Depends(deps.get_db),
) -> Any:
    """
    Get a specific vendors me.
    """
...
def update_vendors(
    *,
    ...
    if not vendors:
        raise HTTPException(
            status_code=404, detail="vendors not found",
        )
