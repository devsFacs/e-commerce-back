...
def read_product_user_views(
    ...
    if crud.user.is_superuser(current_user):
        ...
        count = crud.product_user_view.get_count(
            db=db, user_id=current_user.id, column="id_user"
        )

    response = schemas.ResponseProductUserView(
        **{"count": count, "data": product_user_view}
    )
    return response
...
def read_product_user_view_by_id(
    ...
    if not product_user_view:
        raise HTTPException(status_code=400, detail="ProductUserView not found")
    return product_user_view
...
def update_product_user_view(
    *,
    ...
    if not product_user_view:
        raise HTTPException(
            status_code=404, detail="product_user_view not found",
        )
    product_user_view = crud.product_user_view.update(
        db, db_obj=product_user_view, obj_in=product_user_view_in
    )
    return product_user_view
