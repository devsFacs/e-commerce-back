...
def read_product_user_likes(
    ...
    if crud.user.is_superuser(current_user):
        ...
        count = crud.product_user_like.get_count(db=db, user_id=current_user.id, column='id_user')

    response = schemas.ResponseProductUserLike(**{'count': count, 'data': product_user_like})
    return response
...
def read_product_user_like_by_id(
    ...
    if not product_user_like:
        raise HTTPException(
            status_code=400, detail="ProductUserLike not found"
        )
    return product_user_like
...
def update_product_user_like(
    ...
    if not product_user_like:
        raise HTTPException(
            status_code=404,
            detail="product_user_like not found",
        )
    product_user_like = crud.product_user_like.update(db, db_obj=product_user_like, obj_in=product_user_like_in)
    return product_user_like
